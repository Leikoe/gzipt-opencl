...
fn main() -> Result<()> {
    ...
    let program_source = fs::read_to_string("./src/shoco_compress.cl")
    ...
    let ncds_buffer = unsafe {
        Buffer::<cl_float>::create(&context, CL_MEM_WRITE_ONLY, inputs.len() * inputs.len(), ptr::null_mut())?
    };
    ...
    for _ in 0..N_ITERS {
        ...
        let kernel_event = unsafe {
            ExecuteKernel::new(&kernel)
                ...
                .set_arg(&ncds_buffer)
                .set_global_work_sizes(&[inputs.len(), inputs.len()])
        ...
        let mut results = vec![0.0 as cl_float; inputs.len() * inputs.len()];
        let _read_event =
            unsafe { queue.enqueue_read_buffer(&ncds_buffer, CL_BLOCKING, 0, &mut results, &events)? };
    ...
    println!("average kernel duration (ns): {}", total_duration as f64/N_ITERS as f64);
